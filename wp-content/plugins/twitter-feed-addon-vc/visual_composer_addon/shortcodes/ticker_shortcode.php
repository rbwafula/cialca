...
$visible_tweets = isset($atts['visible_tweets'])?intval($atts['visible_tweets']):intval(2);
$display_controls = isset($atts['display_controls'])?'0':'1';
$ticker_pause_mousehover = isset($atts['ticker_pause_mousehover'])?'1':'0';
$ticker_direction = isset($atts['ticker_direction'])?esc_attr($atts['ticker_direction']):esc_attr('up');
$ticker_speed = isset($atts['ticker_speed'])?intval($atts['ticker_speed']):intval(6000);
$ticker_pause_duration	= isset($atts['ticker_pause_duration'])?intval($atts['ticker_pause_duration']):intval(2500);
...
$temp_custom = array(
			...
			'controls_color'=>isset($atts['controls_color'])?esc_attr($atts['controls_color']):'',
		);
$customization = maybe_serialize($temp_custom);

echo do_shortcode("[twitter-feed-addon-vc-ticker $username template='$twitter_template' data_time_format='$date_time_format' username_link_status='$username_link_status' follow_button='$follow_btn_status' total_feeds='$no_of_tweets' visible_tweets=$visible_tweets display_actions='$display_actions' mouse_pause='$ticker_pause_mousehover' controls='$display_controls' ticker_speed='$ticker_speed' transition_speed='$ticker_pause_duration' ticker_direction='$ticker_direction' image_width='$image_width' image_status='$disable_image_status' customization='$customization' fetch_unavailable='$fetch_unavailable']");

if (isset($_GET['vc_editable'])) {
	if ($_GET['vc_editable'] == true) {
		?>
		<div class="tfa-init-ticker">
			<script type="text/javascript">
				tfaInitTicker(jQuery('.tfa-init-ticker').parent().find('.tfa-pro-ticker-main-wrapper'),true);
			</script>
		</div>
		<?php
	}
