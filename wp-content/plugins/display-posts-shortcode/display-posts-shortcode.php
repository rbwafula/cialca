...
function be_display_posts_shortcode( $atts ) {
	...
	while ( $dps_listing->have_posts() ) :
		...
		if ( $image_size && has_post_thumbnail() && $include_link ) {
			...
			$image = '<a class="image" href="' . get_permalink() . '">'.'<img src="'.get_home_url().'/wp-content/uploads/2019/10/blank_news_image.jpg" alt="'.the_title().'" />' . '</a> ';
		}
...
function be_dps_pagination_links( $output, $atts, $listing ) {
	...
	if( $navigation ) {
    	$nav_output .= '<nav class="display-posts-pagination" role="navigation">';
        	...
        	$nav_output .= '<div class="nav-links text-center">' . $navigation . '</div>';
		$nav_output .= '</nav>';
	}
